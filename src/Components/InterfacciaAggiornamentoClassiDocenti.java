...
public class InterfacciaAggiornamentoClassiDocenti extends JFrame {
    ...
    private void inizializzaUI(ArrayList<Classe> classi, ArrayList<Docente> docenti) {
        setTitle("Gestione Orario Scolastico");
        setSize(1280, 700);
        ...
        JPanel pannelloSinistra = creaPannelloSinistro(classi, docenti);
    ...
    private JPanel creaPannelloSinistro(ArrayList<Classe> classi, ArrayList<Docente> docenti) {
        ...
        if (classeSelezionata != null) pannelloOrario.add(new TabellaOraria(classeSelezionata), BorderLayout.CENTER);
    ...
    private JPanel creaPannelloSelezioneClasse(ArrayList<Classe> classi, ArrayList<Docente> docenti) {
        ...
        // Combo classi
        JPanel pannelloSelezioneClasse = new JPanel(new FlowLayout());
        ...
        classi.forEach(c -> comboClassi.addItem(c.getSezione()));
        pannelloSelezioneClasse.add(labelClasse);
        pannelloSelezioneClasse.add(comboClassi);

        // Combo docenti
        JPanel pannelloSelezioneDocente = new JPanel(new FlowLayout());
        ...
        selezioneClassi.addActionListener(e -> { pannelloSelezioneClasse.setVisible(true); pannelloSelezioneDocente.setVisible(false); aggiornaTabellaClasse(); });
        selezioneDocenti.addActionListener(e -> { pannelloSelezioneClasse.setVisible(false); pannelloSelezioneDocente.setVisible(true); aggiornaTabellaDocente(); });
    ...
    private void aggiornaTabellaClasse() {
        ...
        Classe classe = gestoreDati.getClasseBySezione((String) comboClassi.getSelectedItem());
        if (classe != null) pannelloOrario.add(new TabellaOraria(classe), BorderLayout.CENTER);
    ...
    private void aggiornaTabellaDocente() {
        pannelloOrario.removeAll();
        Docente docente = gestoreDati.getDocenteByCognome((String) comboDocenti.getSelectedItem());
        if (docente != null) pannelloOrario.add(new TabellaOraria(docente), BorderLayout.CENTER);
        pannelloOrario.revalidate();
        pannelloOrario.repaint();
    }
    ...
    private JPanel creaPannelloDestro() {
        ...
        JPanel pannelloPulsanti = new JPanel(new GridLayout(3, 1, 15, 15));
        pannelloPulsanti.setBackground(COLORE_SFONDO);
        pannelloPulsanti.setBorder(BorderFactory.createEmptyBorder(20, 10, 20, 10));

        JButton bAggiorna = new JButton("Aggiorna orario");
        personalizzaBottone(bAggiorna, new Color(70, 180, 130));
        bAggiorna.addActionListener(e -> {
            // Log e aggiornamento eventuali dati
            serializzazione.log("Orario aggiornato per classe: " + comboClassi.getSelectedItem());
            JOptionPane.showMessageDialog(this, "Orario aggiornato con successo!");
        });

        JButton bIndietro = new JButton("Indietro");
        personalizzaBottone(bIndietro, new Color(255, 159, 67));
        bIndietro.addActionListener(e -> { dispose(); new MenuPrincipale(gestoreDati, serializzazione); });

        pannelloPulsanti.add(bAggiorna);
    ...
    private void personalizzaBottone(JButton bottone, Color coloreSfondo) {
        ...
        bottone.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) { bottone.setBackground(coloreSfondo.brighter()); }
            public void mouseExited(java.awt.event.MouseEvent evt) { bottone.setBackground(coloreSfondo); }
        });
    }
}
